...
import org.cognetics.proverspalette.base.cli.ProversPaletteExternalFailure;
...
public class PreviewTabComposite extends org.cognetics.proverspalette.base.gui.PreviewTabComposite {
...
//							outputStyledText.setCaretOffset(msg.length());
		...
		try {
			...
			invocation.addOutputListener(new OutputListener() {
				public void newErrorOutput(String line) {
					//ignore errors (don't think stderr is used with our version of qepcad)
					System.err.println(view.getName()+" gave error output, ignoring: "+line);
				}
	...
	public void update() {
		...
		if (setGoButtonEnablement(null)) {
			try {
				inputStyledText.setText(
						convertFromRealTextToDisplayText(generateQepcadProblem().getQepcadInput(), inputStyledText));
			} catch (QepcadDataException e) {
				System.err.println("unable to generate qepcad problem: "+e);
			}
		}
		noteUpdateCompleted();
	}
...
//	QepcadProblem currentInputQepcadProblem;
	...
	public synchronized ProversPaletteExternalInvocation newExternalInvocation() throws Exception {
		try {
			QepcadProblem problem = generateQepcadProblem();			
			
			if (problem.variablesAndBindings.length==0) {
				return new ProversPaletteExternalFailure("(The current settings are not valid QEPCAD input. \n" +
						"There are no variables to eliminate.)");
			}
			...
			QepcadProcess qp = new QepcadProcess(problem);
			if (inputStyledText.getText().trim().length()>0 &&
					!convertFromDisplayTextToRealText(inputStyledText.getText(), inputStyledText).trim().
					equals(problem.getQepcadInput().trim())) {
				System.out.println("qepcad script looks like it has been editted; sending it anyway");
				qp.setManualInput(
						(convertFromDisplayTextToRealText(inputStyledText.getText(), inputStyledText).
								trim()+"\n").getBytes());
			} else {
				inputStyledText.setText(
						convertFromRealTextToDisplayText( problem.getQepcadInput(), inputStyledText) );
			}
			return qp;
		} catch (Exception e) {
			inputStyledText.setText("");
	...
	private QepcadProblem generateQepcadProblem() throws QepcadDataException {
		QepcadProblem problem = new QepcadProblem();
		((ProblemTabComposite) ((MainQepcadViewPart)view).getProblemComposite()).
			contributeSettingsToQepcadProblem(problem);
		((MainQepcadViewPart)view).getConfigComposite().contributeSettingsToQepcadProblem(problem);
		return problem;
	}

	protected String getAnnotatedResult(ProversPaletteExternalInvocation invocation) {
		if (invocation instanceof QepcadProcess) {
			String qepcadWitness = ((QepcadProcess)invocation).getWitnessMessage();
			if (qepcadWitness!=null) {
